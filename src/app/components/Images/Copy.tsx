"use client";
import { useCopy, useCopyTxt } from "@/app/Services/zustand";
import React from "react";

function Copy() {
  const copyMode = useCopy((state) => state.copy);
  const ChangeCopyMode = useCopy((state) => state.setCopy);
  const CopyText = useCopyTxt((state) => state.setCopyText);
  const CopyTxt = () => {
    CopyText(true);
  };
  ...
  return (
    <>
      <svg
        onClick={CopyTxt}
        onMouseEnter={ChangeCopyMode}
        onMouseLeave={ChangeCopyMode}
        width="21"
        height="24"
        viewBox="0 0 21 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="cursor-pointer max-[510px]:w-[17.5px] max-[510px]:h-[20px]"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
